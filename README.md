...
| --- | --- | --- |
| **Tracer Shim** | The Hook | A lightweight SDK wrapper that intercepts Gemini API calls to capture model ID, token count, and regional headers. |
| **Impact Engine** | The Calculator | An agent-driven service that maps metadata to hardware profiles (TPU v4/v5/v6) and local grid intensities. |
| **Sustainability Auditor** | The Reporter | Aggregates data into a TimescaleDB time-series database and generates compliance-ready reports. |
...
Monitoring a Gemini API call is as simple as adding the `@leaftrail` decorator to your functions.

```python
from dotenv import load_dotenv
from google import genai
from leaftrail import leaftrail
import os
